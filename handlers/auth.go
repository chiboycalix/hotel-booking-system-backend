...
func RegisterUser(c *fiber.Ctx) error {
	var userCollection = common.GetDBCollection(USERS_MODEL)
	var u createUserDTO
	u.IsVerified = true
	u.Role = "GUEST"
	...
	if err == nil {
		return c.Status(http.StatusConflict).JSON(responses.APIResponse{Status: http.StatusConflict, Message: "Already Exist", Data: &fiber.Map{"error": "User with this Email already exist"}})
	}

	pass, hashErr := utils.HashPassword(u.Password)
	if hashErr != nil {
		return c.Status(http.StatusBadRequest).JSON(responses.APIResponse{Status: http.StatusBadRequest, Message: "Failed to hash password", Data: &fiber.Map{"error": hashErr.Error()}})
	}
	u.Password = pass
	...
	if err != nil {
		return c.Status(http.StatusInternalServerError).JSON(responses.APIResponse{Status: http.StatusInternalServerError, Message: "Failed to generate jwt", Data: &fiber.Map{"error": err.Error()}})
	}

	return c.Status(http.StatusCreated).JSON(responses.APIResponse{Status: http.StatusCreated, Message: "User created successfully", Data: &fiber.Map{"user": result, "token": jwt}})
}
...
func LoginUser(c *fiber.Ctx) error {
	...
	if err := c.BodyParser(&l); err != nil {
		return c.Status(http.StatusBadRequest).JSON(responses.APIResponse{Status: http.StatusBadRequest, Message: "Invalid Body", Data: &fiber.Map{"error": err.Error()}})
	}
	...
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(responses.APIResponse{Status: http.StatusBadRequest, Message: "User not found", Data: &fiber.Map{"error": err.Error()}})
	}

	if err := utils.CheckPasswordHash(result.Password, l.Password); err != nil {
		return c.Status(http.StatusBadRequest).JSON(responses.APIResponse{Status: http.StatusBadRequest, Message: "Invalid credentials", Data: &fiber.Map{"error": "Invalid Email or Password"}})
	}

	if !result.IsVerified {
		return c.Status(http.StatusBadRequest).JSON(responses.APIResponse{Status: http.StatusBadRequest, Message: "Not Verified", Data: &fiber.Map{"error": "User is not Verified"}})
	}
	...
	return c.Status(http.StatusOK).JSON(responses.APIResponse{Status: http.StatusOK, Message: "Login successful", Data: &fiber.Map{"token": jwt}})
}
...
func ForgetPassword(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(responses.APIResponse{Status: http.StatusBadRequest, Message: "User not found", Data: &fiber.Map{"error": "User with this email not found"}})
	}

	// send email
	err = utils.SendMailService(result, "templates/forget-password.html", "Forget Password")
	if err != nil {
		return c.Status(http.StatusInternalServerError).JSON(responses.APIResponse{Status: http.StatusInternalServerError, Message: "Error sending mail", Data: &fiber.Map{"error": "Something went wrong, error sending mail"}})
	}

	return c.Status(http.StatusOK).JSON(responses.APIResponse{Status: http.StatusOK, Message: "Please check your mail for further instructions"})
}
...
func ResetPassword(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(responses.APIResponse{Status: http.StatusBadRequest, Message: "User not found", Data: &fiber.Map{"error": "User not found"}})
	}
	...
	if err != nil {
		return c.Status(http.StatusInternalServerError).JSON(responses.APIResponse{Status: http.StatusInternalServerError, Message: "Error sending mail", Data: &fiber.Map{"error": err.Error()}})
	}

	return c.Status(http.StatusOK).JSON(responses.APIResponse{Status: http.StatusOK, Message: "Password reset was successful", Data: &fiber.Map{"user": updateReq}})
}
...
func VerifyAccount(c *fiber.Ctx) error {
	...
	return c.Status(http.StatusOK).JSON(responses.APIResponse{Status: http.StatusOK, Message: "Your account has been verified", Data: &fiber.Map{"user": updateReq}})
}
...
func GoogleCallback(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(http.StatusInternalServerError).JSON(responses.APIResponse{Status: http.StatusInternalServerError, Message: "Error getting User data", Data: &fiber.Map{"error": "Something went wrong, getting User data"}})
	}
